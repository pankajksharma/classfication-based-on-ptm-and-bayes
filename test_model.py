...
categories = ["TOPICS", "ORGS", "EXCHANGES", "COMPANIES", "PLACES", "PEOPLE"]
...
category_counts = {
				...
				"COMPANIES" : json.load(open(MODEL_DIR+"companies.json")), 
...
while True:
	...
	if not c:
		break
	# print c[0]
	probability = {"TOPICS":1, "ORGS":1, "EXCHANGES":1, "COMPANIES":1, "PLACES":1, "PEOPLE":1}
	cats = json.loads(c[0])
	...
	for words in sents:
		for word in words:
			...
			if foureir_applied:
				for cat in categories:
					try:
						probability[cat] *= log((category_counts[cat][word]+1)/(counts[cat]+1))
					except:
						probability[cat] *= log(1.0/(counts[cat]+1))
			else:
				for cat in categories:
					probability[cat] *= log(category_counts[cat][word]/counts[cat])
	print probability,cats